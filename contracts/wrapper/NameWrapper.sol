...
import "../memeregistrar/IBaseRegistrar.sol";
    ...
    IERC721Receiver
{
    ...
    /**
     * @notice Sets records for the subdomain in the MNS Registry
    ...
    {
        mns.setSubnodeRecord(parentNode, label, owner, resolver, ttl);
    }
    ...
    function setSubnodeOwner(
        ...
        returns (bytes32)
    {
        return mns.setSubnodeOwner(parentNode, label, owner);
    }
    ...
    {
        mns.setRecord(node, owner, resolver, ttl);
    }
    ...
    modifier canCallSetSubnodeOwner(bytes32 node, bytes32 label) {
        ...
        address owner = mns.owner(subnode);
        (, uint96 fuses) = getData(uint256(node));
    ...
    function _checkOwnership(
        ...
        if (mns.owner(node) != address(this)) {
